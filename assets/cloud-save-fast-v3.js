(() => {
  ...
  if (!window.__ciRawFetch) window.__ciRawFetch = window.fetch.bind(window);
  const nativeFetch = window.__ciRawFetch;
  window.__ciCloudBaselineLS = window.__ciCloudBaselineLS || {};
  const asUrl = (input) => {
    try {
  ...
  const diffLocal = (next) => {
    const base = window.__ciCloudBaselineLS || {};
    const changed = {};
  ...
  const save = async (data) => {
    ...
    let changedLocalStorage = diffLocal(localStorageData);
    if (!Object.keys(changedLocalStorage).length) {
      changedLocalStorage = localStorageData;
    }
    const patch = { localStorage: changedLocalStorage, updatedAt: new Date().toISOString() };
    ...
    window.__ciCloudBaselineLS = { ...(window.__ciCloudBaselineLS || {}), ...changedLocalStorage };
    return json({
      ok: true,
      success: true,
      verified: true,
  ...
  const rows = (state) => {
    ...
    window.__ciCloudBaselineLS = { ...localStorageData };
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (method === 'POST' && url === `${API}/api/state`) {
      try {
        const body = readBody(init);
        return await save(body.data || body || {});
      } catch (error) {
